 ...
 * 制动试验淋水箱
 */
public class BrakeTestShowerTankImpl implements AnalysisFactory {
    @Override
    public Map importExcel(XSSFSheet xssfSheet, Map map, List<VSourceRecord> list) throws BusinessRuntimeException {
        ...
        for (int i = 16; i < 21; i++) {
            ...
            try{
                XSSFRow xssfRow = xssfSheet.getRow(i);
                XSSFCell setValue = xssfRow.getCell(2);
                ...
                if (StringUtils.isNotEmpty(standardValues + "") && StringUtils.isNotEmpty(setValue + "") && StringUtils.isNotEmpty(intrinsicError + "")) {

                    Matcher standardValuesIsNum = pattern.matcher(standardValues + "");
                    ...
                    record.setStandardValues(Float.valueOf(standardValues + ""));
    ...
    public Map exportExcel(List<VSourceRecord> sourceList, Map<String, Object> resultMap) throws Exception {
        ...
        resultMap.put("filePath", "制动试验淋水箱.xlsx");
        return resultMap;
    }

    @Override
    public AnalysisFactory getAnalysis() {
        return new BrakeTestShowerTankImpl();
    }
}
