 ...
 * 求积仪
 */
public class PlanimeterImpl  implements AnalysisFactory {
    @Override
    public Map importExcel(XSSFSheet xssfSheet, Map map, List<VSourceRecord> list)  throws BusinessRuntimeException {
        ...
        for (int i =16; i <17; i++) {
			try{
				...
				if( StringUtils.isNotEmpty(setValue+"") &&  StringUtils.isNotEmpty(averageValue+"")){
...
//        String tmpFile = "classpath:static/求积仪.xls";
//        File file = ResourceUtils.getFile(tmpFile);
        resultMap.put("filePath","求积仪.xls");
        return resultMap;
    }

    @Override
    public AnalysisFactory getAnalysis() {
        return new PlanimeterImpl();
    }
}
