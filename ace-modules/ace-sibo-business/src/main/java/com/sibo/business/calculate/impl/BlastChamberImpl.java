 ...
 * 鼓风试验箱
 */
public class BlastChamberImpl implements AnalysisFactory {
    @Override
    public Map importExcel(XSSFSheet xssfSheet, Map map, List<VSourceRecord> list) throws BusinessRuntimeException {
        ...
        for (int i = 12; i < 13; i++) {
            try{
                XSSFRow xssfRow = xssfSheet.getRow(i);
                XSSFCell standarValue = xssfRow.getCell(1);
                double averageValue = xssfRow.getCell(11).getNumericCellValue();
                Pattern pattern = Pattern.compile("-[0-9]+(.[0-9]+)?|[0-9]+(.[0-9]+)?");
                if (StringUtils.isNotEmpty(averageValue + "") && StringUtils.isNotEmpty(standarValue + "")) {
        ...
        //计量日期
        for (int i = 30; i <= xssfSheet.getLastRowNum(); i++) {
			try{
    ...
    public Map<String, Object> exportExcel(List<VSourceRecord> sourceList, Map<String, Object> datas) throws Exception {
        ...
        datas.put("filePath", "鼓风试验箱.xlsx");
        return datas;
    }

    @Override
    public AnalysisFactory getAnalysis() {
        return new BlastChamberImpl();
    }
}
