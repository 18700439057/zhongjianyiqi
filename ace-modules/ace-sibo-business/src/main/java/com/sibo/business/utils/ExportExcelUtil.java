...
public class ExportExcelUtil<T>{
    ...
    public void exportExcel2007(String title, String[] headers, Collection<T> dataset, OutputStream out, String pattern) {
        ...
        try {
//            writeExcelToDisk(target,workbook);
            workbook.write(out);
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     *            如果有时间数据，设定输出格式。默认为"yyyy-MM-dd hh:mm:ss"
     */
    public void exportExcel2007(String title, String[] headers, Collection<T> dataset, OutputStream out, String pattern,JSONArray jsonArray) {
        // 声明一个工作薄
        XSSFWorkbook workbook = new XSSFWorkbook();
        // 生成一个表格
        XSSFSheet sheet = workbook.createSheet(title);
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                for (int j = 0; j < jsonArray.size(); j++){
                    ...
                    String defaultFiled = "get"+(jsonObject.getString("value").substring(0, 1).toUpperCase())+jsonObject.getString("value").substring(1);
                    if(getMethodName.equals(defaultFiled)){
                        col = j;
                        flag = true;
                    }
                }
...
//            writeExcelToDisk(target,workbook);
     ...
     * @param wb
     */
    private void writeExcelToDisk(String filePath, XSSFWorkbook wb) {
        try {
