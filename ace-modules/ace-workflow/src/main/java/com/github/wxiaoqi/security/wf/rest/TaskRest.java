...
//    @GetMapping("/todo")
    ...
    public List getMyApproveTasks(int pageNum, int pageSize) {
        if (pageNum <= 1) {
            pageNum = 1;
        }
        if (pageSize <= 1) {
        ...
        System.out.print("业务key1:============================================================");
        List<String> result = new ArrayList<>();
        tasks.forEach(task -> {
            ProcessInstance pi =  runtimeService.createProcessInstanceQuery().processInstanceId(task.getProcessInstanceId()).singleResult();
            System.out.print("业务key1:"+pi.getBusinessKey());
            result.add(pi.getBusinessKey());
        });
        // 获取岗位待签收任务
        ...
        if (groups != null && groups.size() > 0) {
            ...
            tasks.forEach(task -> {
                ProcessInstance pi =  runtimeService.createProcessInstanceQuery().processInstanceId(task.getProcessInstanceId()).singleResult();
                System.out.print("业务key2:"+pi.getBusinessKey());
//                result.add(task.toString());\
                result.add(pi.getBusinessKey());
            });
        }
        ...
        tasks.forEach(task -> {
            ProcessInstance pi =  runtimeService.createProcessInstanceQuery().processInstanceId(task.getProcessInstanceId()).singleResult();
            System.out.print("业务key3:"+pi.getBusinessKey());
            result.add(task.toString());
        });
//        return new TableResultResponse(count, result);
        return result;
