...
public class FlowService {
    ...
    public ExecutionEntity createProcess(String processName,Map<String,Object> map){
        ...
        // 启动一个流程实例
        pi = (ExecutionEntity) runtimeService.startProcessInstanceByKey(processName,"1", map);
    ...
    public ExecutionEntity createProcess(String processName,Map<String,Object> map,String maintainId){
        ExecutionEntity pi = null;
        //流程启动
        identityService.setAuthenticatedUserId(BaseContextHandler.getUsername());
        // 启动一个流程实例
        pi = (ExecutionEntity) runtimeService.startProcessInstanceByKey(processName,maintainId, map);
        String taskId = pi.getTasks().get(0).getId();
        // 根据实例把流程往下推
        taskService.complete(taskId, map);
        return pi;
