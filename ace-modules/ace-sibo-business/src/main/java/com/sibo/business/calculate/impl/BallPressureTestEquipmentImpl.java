 ...
 * 球压试验装置
 */
public class BallPressureTestEquipmentImpl implements AnalysisFactory {
    @Override
    ...
    public Map<String, Object> exportExcel(List<VSourceRecord> sourceList,Map<String, Object> datas) throws Exception {
        ...
        for (VSourceRecord vSourceRecord : sourceList) {
            String name = vSourceRecord.getFormula().substring(0,2);
            if(name.equals("压力")){
        ...
        datas.put("filePath","球压试验装置.xlsx");
        return datas;
    }

    @Override
    public AnalysisFactory getAnalysis() {
        return new BallPressureTestEquipmentImpl();
    }
}
